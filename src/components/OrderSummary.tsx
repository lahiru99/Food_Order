...
export function OrderSummary({
  ...
  const getPackageDishes = (description?: string): string[] => {
    ...
    const match = description.match(
      /Package includes: (.*?)(?:\. Also includes|$)/
    );
    ...
    return match[1]
      .split(', ')
      .map((dish) => dish.replace(/^and /, '').trim())
      .filter(Boolean);
  };
        ...
        <>
          <ul className="order-items">
            {filteredItems.map((item, index) => {
              ...
              const packageDishes = isPackage
                ? getPackageDishes(item.description)
                : [];
              ...
              return (
                <li
                  key={index}
                  className={`order-item ${isPackage ? 'package-item' : ''}`}
                >
          ...
          >
            {isDeadlinePassed ? 'Order deadline passed' : 'Proceed to Checkout'}
          </button>
        </>
      ) : (
        <div className="empty">
