...
import { getOrders, getOrderSettings, getMenu, updateMenuItem, deleteMenuItem, addMenuItem } from '../services/firebase';
...
export function AdminPage() {
  console.log('AdminPage component loaded, dummyOrders:', dummyOrders.length, 'orders');
      ...
      try {
        if (activeTab === 'menu') {
          ...
          if (ordersData && ordersData.length > 0) {
            ...
            console.log('No orders found or Firebase failed, using dummy orders');
            setOrders(dummyOrders);
            setUseDummyData(true);
          }
        }
      } catch (err) {
        console.error(`Error fetching ${activeTab} data, using dummy data:`, err);
        
        if (activeTab === 'orders') {
          console.log('Setting dummy orders due to error:', dummyOrders.length, 'orders');
          setOrders(dummyOrders);
        }
        setUseDummyData(true);
      } finally {
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    return menuItems.filter(item => 
      ...
      (item.nameLocalLang && item.nameLocalLang.toLowerCase().includes(search)) ||
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input 
                ...
                onChange={e => setNewItem({...newItem, nameLocalLang: e.target.value})}
                        ...
                        {editingItem?.id === item.id ? (
                          ...
                          <span className={`category-badge ${item.category}`}>
                            {item.category === 'non-veg' ? 'Non-Veg' : 
                             ...
                             item.category === 'extras' ? 'Extras' : 'Other'}
                          </span>
                        )}
                      </td>
                      <td>
    ...
    <div className="admin-page">
      <header>
        ...
        <div className="search-bar">
          <input
            type="text"
            placeholder={`Search ${activeTab === 'orders' ? 'orders' : 'menu items'}...`}
            value={searchTerm}
      ...
      {isWhatsAppModalOpen && selectedOrder && (
        <>
          <div className="modal-overlay" onClick={handleCloseWhatsAppModal}></div>
